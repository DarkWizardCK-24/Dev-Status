...
export default function AuthButton() {
  ...
  function signIn() {
    const devfolioUrl = process.env.NEXT_PUBLIC_DEVFOLIO_URL;
    const callbackUrl = `${window.location.origin}/api/auth/callback`;
    const state = window.location.pathname;
    if (devfolioUrl) {
      window.location.href = `${devfolioUrl}/api/auth/cross-app?redirect_to=${encodeURIComponent(callbackUrl)}&state=${encodeURIComponent(state)}`;
    } else {
